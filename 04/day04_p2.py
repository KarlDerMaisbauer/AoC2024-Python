...
def diag1(sub_matrix):
    if sub_matrix[0][0] == 'M' and sub_matrix[1][1] == 'A' and sub_matrix[2][2] == 'S':
        return True
    if sub_matrix[2][2] == 'M' and sub_matrix[1][1] == 'A' and sub_matrix[0][0] == 'S':
        return True
    return False


def diag2(sub_matrix):
    if sub_matrix[2][0] == 'M' and sub_matrix[1][1] == 'A' and sub_matrix[0][2] == 'S':
        return True
    if sub_matrix[0][2] == 'M' and sub_matrix[1][1] == 'A' and sub_matrix[2][0] == 'S':
        return True
    return False
...
def check_sub_matrix(sub_matrix):
    ...
    assert (len(sub_matrix) == 3)
    assert (len(sub_matrix[0]) == 3)
    if diag1(sub_matrix) and diag2(sub_matrix):
        return 1
    else:
        return 0
    return counter
...
with open("input.txt", 'r') as file:
    ...
    for col in range(len(matrix) - 2):
        for row in range(len(matrix[0]) - 2):
            counter += check_sub_matrix([row[col:col+3]
                                         for row in matrix[row:row+3]])
